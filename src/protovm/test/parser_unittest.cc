...
TEST_F(ParserTest, NoInstructions) {
  EXPECT_CALL(executor,
              EnterField(testing::Field(&Cursors::selected, CursorEnum::SRC),
                         testing::_))
      .Times(0);
  ...
  EXPECT_CALL(executor, ReadRegister(testing::_)).Times(0);

  auto program = SamplePrograms::NoInstructions().SerializeAsString();
  Parser interpreter(AsConstBytes(program), executor);
  ASSERT_TRUE(interpreter.Run(RoCursor{}, RwProto::Cursor{}).IsOk());
}

TEST_F(ParserTest, RegLoad) {
  EXPECT_CALL(executor, WriteRegister(testing::_, testing::_))
...
TEST_F(ParserTest, Del) {
  EXPECT_CALL(executor, Delete(testing::_))
      .WillOnce(testing::Return(testing::ByMove(StatusOr<void>::Ok())));
...
TEST_F(ParserTest, Merge) {
  EXPECT_CALL(executor, Merge(testing::_))
      .WillOnce(testing::Return(testing::ByMove(StatusOr<void>::Ok())));
...
TEST_F(ParserTest, Set) {
  EXPECT_CALL(executor, Set(testing::_))
      .WillOnce(testing::Return(testing::ByMove(StatusOr<void>::Ok())));
...
TEST_F(ParserTest, Select_AllCursorTypes) {
  {
    ...
    EXPECT_CALL(
        executor,
        EnterField(testing::Field(&Cursors::selected, CursorEnum::SRC), 1))
        .WillOnce(testing::Return(testing::ByMove(StatusOr<void>::Ok())));

    // SRC
    EXPECT_CALL(
        executor,
        EnterField(testing::Field(&Cursors::selected, CursorEnum::SRC), 2))
        .WillOnce(testing::Return(testing::ByMove(StatusOr<void>::Ok())));

    // DST
    EXPECT_CALL(
        executor,
        EnterField(testing::Field(&Cursors::selected, CursorEnum::DST), 3))
        .WillOnce(testing::Return(testing::ByMove(StatusOr<void>::Ok())));
  }
...
TEST_F(ParserTest, Select_AccessAllFieldTypes) {
...

  {
    ...
    // enter field
    EXPECT_CALL(executor, EnterField(testing::_, testing::_))
        .WillOnce(testing::Return(testing::ByMove(StatusOr<void>::Ok())));
    ...
    EXPECT_CALL(executor, ReadRegister(testing::_))
        .WillOnce(testing::Return(testing::ByMove(StatusOr<uint64_t>(0ul))));

    EXPECT_CALL(executor, EnterRepeatedFieldByKey(testing::_, testing::_,
                                                  testing::_, testing::_))
        .WillOnce(testing::Return(testing::ByMove(StatusOr<void>::Ok())));
...
TEST_F(ParserTest, Select_ExecutesNestedInstructions) {
...

  {
    ...
    // repeated field #1
    {
      ...
      EXPECT_CALL(executor, WriteRegister(testing::_, 10))
          .WillOnce(testing::Return(testing::ByMove(StatusOr<void>::Ok())));

      // nested instruction #2
      EXPECT_CALL(executor, WriteRegister(testing::_, 11))
          .WillOnce(testing::Return(testing::ByMove(StatusOr<void>::Ok())));
    }
...
TEST_F(ParserTest, Select_CanBreakOuterNestedInstructions) {
  {
    ...
    EXPECT_CALL(executor, WriteRegister(testing::_, testing::_)).Times(0);
  }
...
TEST_F(ParserTest, AbortLevel_ABORT) {
  ...
  Parser interpreter(AsConstBytes(program), executor);
  ASSERT_TRUE(interpreter.Run(RoCursor{}, RwProto::Cursor{}).IsAbort());
}
