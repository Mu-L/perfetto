...
// }
//
// IntrusiveList<Person, Person::Traits> list;
// Person person;
...
namespace internal::intrusive_list {
...
// Base class to factor out type-independent code (avoid binary bloat)
class Base {
 public:
...
using IntrusiveListNode = internal::intrusive_list::Node;
...
class IntrusiveList : private internal::intrusive_list::Base {
 public:
  class Iterator {
   public:
    Iterator() = default;
  ...
  void PushFront(T& entry) {
    internal::intrusive_list::Base::PushFront(nodeof(&entry));
  }

  void PopFront() { internal::intrusive_list::Base::PopFront(); }
  ...
  void Erase(T& entry) {
    internal::intrusive_list::Base::Erase(nodeof(&entry));
  }
 ...
 private:
  static constexpr size_t kNodeOffset = Traits::NodeOffset();
